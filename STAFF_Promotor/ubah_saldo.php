...
$logFilePath = $logFileDir . 'debug_staff_saldo_ops.log';
        ...
        if (empty($id_sigma_anggota)) {
             echo '
                <script>
                    ...
                    window.location.replace("' . $alamat_staff . 'saldo");
                </script>
            ';
            exit();
        }
...
if (isset($_POST['ubah_data'])) {
    ...
    if ($jumlah_transaksi <= 0) {
        echo '
            <script>
                ...
                window.location.replace("' . $alamat_staff . 'ubah_saldo/' . $id_anggota . '");
            </script>
        ';
        exit();
    }
    ...
    $debug_log_entry .= "[" . date("Y-m-d H:i:s") . "] ==== staff SALDO OPERATION DEBUG ====\n";
    ...
    if ($api_transaksi_response && $api_transaksi_response['success']) {
        ...
        if ($saldo_final_untuk_db !== null) {
            ...
            file_put_contents($logFilePath, $debug_log_entry . "==== DEBUG END ====\n\n", FILE_APPEND | LOCK_EX);
            echo '<script>alert("' . $pesan_kritis_getbalance . '"); window.location.replace("' . $alamat_staff . 'ubah_saldo/' . $id_anggota . '");</script>';
        }
    } else {
    ...
    <div class="row gy-4 mb-4">
        ...
        <div class="col-md-6">
            <div class="text-center text-md-end">
                <a href="<?php echo htmlspecialchars($alamat_staff . 'saldo'); ?>" class="btn btn-sm btn-primary waves-effect waves-light">
