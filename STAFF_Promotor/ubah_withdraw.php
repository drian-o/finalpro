...
$logFilePath = $logFileDir . 'debug_staff_withdraw_status_change.log';
...
if (isset($_POST['ubah_data'])) {
    ...
    if ($status_withdraw_baru != $status_withdraw_sebelumnya) {
        $debug_log_entry .= "[" . date("Y-m-d H:i:s") . "] ==== staff WITHDRAW STATUS CHANGE ====\n";
        ...
        try {
            ...
            echo '<script>alert("Status withdraw berhasil diubah menjadi '.$status_withdraw_baru.' dan saldo dikembalikan."); window.location.replace("'.$alamat_staff.'withdraw");</script>';
            ...
            echo '<script>alert("Terjadi kesalahan saat memproses withdraw: '.htmlspecialchars($e->getMessage()).'"); window.location.replace("'.$alamat_staff.'ubah_withdraw/'.$id_withdraw.'");</script>';
        }

    } else {
         echo '<script>alert("Tidak ada perubahan status, tidak ada tindakan yang diambil."); window.location.replace("'.$alamat_staff.'withdraw");</script>';
    }
    exit();
} else if (isset($_POST['hapus_data'])) {
    $staff_code = $_SESSION['kode_staff'] ?? 'UNKNOWN_staff';
    $debug_log_entry = "[" . date("Y-m-d H:i:s") . "] ==== staff WITHDRAW DELETE ATTEMPT (LOCAL ONLY) ====\n";
    $debug_log_entry .= "staff: " . $staff_code . "\n";
    $debug_log_entry .= "Withdraw ID: " . $id_withdraw . "\n";
    $debug_log_entry .= "Target User: " . $nama_pengguna_anggota_withdraw . "\n";
    $debug_log_entry .= "Jumlah Withdraw: " . $jumlah_withdraw_display . "\n";
    ...
    if ($stmt_hapus->execute()) {
        ...
        file_put_contents($logFilePath, $debug_log_entry . "==== DEBUG END ====\n\n", FILE_APPEND | LOCK_EX);
        echo '<script>alert("Berhasil hapus data withdraw dari sistem lokal."); window.location.replace("'.$alamat_staff.'withdraw");</script>';
    } else {
    ...
    <div class="row gy-4 mb-4">
        ...
        <div class="col-md-6">
            <div class="text-center text-md-end">
                <a href="<?php echo htmlspecialchars($alamat_staff.'withdraw'); ?>" class="btn btn-sm btn-primary waves-effect waves-light">
