...
// Ambil tanggal dari URL jika ada, jika tidak, gunakan hari ini
                            ...
                            <div class="form-group">
                                ...
                                <div class="flex items-center w-full">
                                    <div class="border border-separator px-3 py-2 w-1/2 lg:w-full lg:h-11 justify-between rounded-lg">
                                        <input id="startDate" class="bg-transparent text-sm w-full focus:outline-none border-none" placeholder="Tanggal Mulai" type="date" value="<?php echo $start_date; ?>">
                                    </div>
                                    <span class="text-sm px-2">-</span>
                                    <div class="border border-separator px-3 py-2 w-1/2 lg:w-full justify-between rounded-lg">
                                        <input id="endDate" class="bg-transparent text-sm w-full focus:outline-none border-none" placeholder="Tanggal Selesai" type="date" value="<?php echo $end_date; ?>">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="w-full lg:pr-2">
                                ...
                                <div class="relative">
                                    <div id="dropdown" class="border border-separator px-3 py-2 flex items-center w-full lg:h-11 justify-between rounded-lg cursor-pointer">
                                        <span id="selectedOption" class="text-sm">Deposit</span>
                                    ...
                                    <div id="dropdown-menu" class="absolute z-[9999] left-0 right-0 top-12 overflow-hidden bg-background-secondary rounded-xl transition-all duration-300 ease-in-out max-h-0">
                                        <div>
                                            <a href="#" class="dropdown-item text-sm border-b border-inverse py-3 block cursor-pointer hover:bg-background-default transition-all duration-300 ease-in-out" data-value="deposit">Deposit</a>
                                            <a href="#" class="dropdown-item text-sm py-3 block cursor-pointer hover:bg-background-default transition-all duration-300 ease-in-out" data-value="withdraw">Withdraw</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let currentTransactionType = 'deposit'; // Nilai default

        // Fungsi untuk memperbarui tampilan tombol filter aktif
        ...
        // Fungsi untuk mengambil dan menampilkan data menggunakan AJAX
        function fetchTransactions(startDate, endDate, transactionType) {
            // Tampilkan pesan loading
            listContainer.innerHTML = '<p class="text-center mt-3">Memuat riwayat transaksi...</p>';
            
            const url = `ajax/get_history_deposit.php?start_date=${startDate}&end_date=${endDate}&transaction_type=${transactionType}`;
        ...
        // Event listener untuk tombol filter cepat
        ...
        dropdown.addEventListener('click', function() {
            dropdownMenu.style.maxHeight = dropdownMenu.style.maxHeight === '0px' ? '100px' : '0px';
        });
        ...
        dropdownItems.forEach(item => {
            item.addEventListener('click', function(e) {
                ...
                currentTransactionType = this.getAttribute('data-value'); // Simpan jenis transaksi yang dipilih
                dropdownMenu.style.maxHeight = '0px';
                // Tidak perlu langsung fetch, menunggu tombol "Tampilkan Data" diklik
            });
        });
        ...
        filterButton.addEventListener('click', function() {
            ...
            fetchTransactions(startDate, endDate, currentTransactionType);
        });

        // Atur nilai default saat halaman dimuat
        const initialStartDate = '<?php echo $start_date; ?>';
        const initialEndDate = '<?php echo $end_date; ?>';
        const initialTransactionType = '<?php echo $transaction_type; ?>';
        
        startDateInput.value = initialStartDate;
        endDateInput.value = initialEndDate;
        
        // Atur tampilan tombol filter yang aktif saat memuat halaman
        const today = new Date();
        const oneDay = 24 * 60 * 60 * 1000;
        const diffDays = Math.round(Math.abs((new Date(initialEndDate) - new Date(initialStartDate)) / oneDay));

        if (initialStartDate === formatDate(today) && initialEndDate === formatDate(today)) {
            updateActiveButton(todayFilter);
        } else if (diffDays === 1 && new Date(initialEndDate).getDate() === today.getDate() && new Date(initialStartDate).getDate() === today.getDate() - 1) {
            updateActiveButton(yesterdayFilter);
        } else if (diffDays === 29) {
            updateActiveButton(last30DaysFilter);
        }
