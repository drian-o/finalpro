...
$game_type = isset($_GET['game_type']) ? mysqli_real_escape_string($koneksi, $_GET['game_type']) : 'table';
...
if (empty($provider_code_from_req)) {
    if ($provider_code_from_req === 'ALL') {
        $response_data['providerName'] = 'Semua GameXa Table Games';
    } else {
    ...
    if ($provider_name_query && mysqli_num_rows($provider_name_query) > 0) {
        ...
        $response_data['message'] = 'Provider not found in database for this type (table).';
        echo json_encode($response_data);
        exit();
    }
}
...
try {
    ...
    error_log("AJAX table_gamelist error: " . $e->getMessage());
}
