...
try {
    // Pastikan tabel arcade_gamelist sudah ada di database Anda
    $count_query = "SELECT COUNT(*) AS total_games
                    FROM arcade_gamelist ag
                    LEFT JOIN srg_gamelist srg ON ag.game_code = srg.game_code AND ag.provider_code = srg.provider_code AND ag.game_source = 'srg'
                    WHERE ag.is_featured = TRUE AND ag.game_type = '{$page_game_type}' AND ag.game_source = 'srg'";
    
    if (!empty($search_term)) {
        $count_query .= " AND (ag.game_name LIKE '%{$search_term}%' OR ag.game_code LIKE '%{$search_term}%')";
    }
    ...
    $query_featured_games = "SELECT ag.game_code, ag.provider_code, ag.game_source, ag.game_name, ag.game_type, ag.custom_image_path,
    ...
    if (!empty($search_term)) {
        $query_featured_games .= " AND (ag.game_name LIKE '%{$search_term}%' OR ag.game_code LIKE '%{$search_term}%')";
    }
    
    $query_featured_games .= " ORDER BY ag.display_order ASC, ag.game_name ASC LIMIT {$limit} OFFSET {$offset}";
    ...
    error_log("AJAX featured_arcade_gamelist error: " . $e->getMessage());
}
