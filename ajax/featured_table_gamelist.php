...
try {
    // Pastikan tabel table_gamelist sudah ada di database Anda
    $count_query = "SELECT COUNT(*) AS total_games
                    FROM table_gamelist tg
                    LEFT JOIN srg_gamelist srg ON tg.game_code = srg.game_code AND tg.provider_code = srg.provider_code AND tg.game_source = 'srg'
                    WHERE tg.is_featured = TRUE AND tg.game_type = '{$page_game_type}' AND tg.game_source = 'srg'";
    
    if (!empty($search_term)) {
        $count_query .= " AND (tg.game_name LIKE '%{$search_term}%' OR tg.game_code LIKE '%{$search_term}%')";
    }
    ...
    $query_featured_games = "SELECT tg.game_code, tg.provider_code, tg.game_source, tg.game_name, tg.game_type, tg.custom_image_path,
    ...
    if (!empty($search_term)) {
        $query_featured_games .= " AND (tg.game_name LIKE '%{$search_term}%' OR tg.game_code LIKE '%{$search_term}%')";
    }
    
    $query_featured_games .= " ORDER BY tg.display_order ASC, tg.game_name ASC LIMIT {$limit} OFFSET {$offset}";
    ...
    error_log("AJAX featured_table_gamelist error: " . $e->getMessage());
}
