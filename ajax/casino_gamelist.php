...
$game_type = isset($_GET['game_type']) ? mysqli_real_escape_string($koneksi, $_GET['game_type']) : 'casino';
...
if (empty($provider_code_from_req)) {
    if ($provider_code_from_req === 'ALL') {
        $response_data['providerName'] = 'Semua GameXa Live Casino';
    } else {
    ...
    $provider_name_query = mysqli_query($koneksi, "SELECT provider_name FROM srg_provider WHERE provider_code = '{$provider_code_from_req}' AND provider_type = '{$game_type}' LIMIT 1");
    if ($provider_name_query && mysqli_num_rows($provider_name_query) > 0) {
        ...
        $response_data['message'] = 'Provider not found in database for this type (casino).';
        echo json_encode($response_data);
        exit();
    }
}
...
try {
    $count_query = "SELECT COUNT(*) AS total_games FROM srg_gamelist WHERE game_type = '{$game_type}' AND game_status = 'active'";
    if ($provider_code_from_req !== 'ALL') {
    ...
    $query_games = "SELECT game_code, game_name, game_image_local, game_image_url_api, game_status, provider_code FROM srg_gamelist WHERE game_type = '{$game_type}'";
    if ($provider_code_from_req !== 'ALL') {
    ...
    $query_games .= " AND game_status = 'active'";
    $query_games .= " ORDER BY game_name ASC LIMIT {$limit} OFFSET {$offset}";
    ...
    error_log("AJAX casino_gamelist error: " . $e->getMessage());
}
