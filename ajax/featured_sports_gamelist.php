...
try {
    // Pastikan tabel sports_gamelist sudah ada di database Anda
    $count_query = "SELECT COUNT(*) AS total_games
                    FROM sports_gamelist spg
                    LEFT JOIN srg_gamelist srg ON spg.game_code = srg.game_code AND spg.provider_code = srg.provider_code AND spg.game_source = 'srg'
                    WHERE spg.is_featured = TRUE AND spg.game_type = '{$page_game_type}' AND spg.game_source = 'srg'";
    
    if (!empty($search_term)) {
        $count_query .= " AND (spg.game_name LIKE '%{$search_term}%' OR spg.game_code LIKE '%{$search_term}%')";
    }
    ...
    $query_featured_games = "SELECT spg.game_code, spg.provider_code, spg.game_source, spg.game_name, spg.game_type, spg.custom_image_path,
    ...
    if (!empty($search_term)) {
        $query_featured_games .= " AND (spg.game_name LIKE '%{$search_term}%' OR spg.game_code LIKE '%{$search_term}%')";
    }
    
    $query_featured_games .= " ORDER BY spg.display_order ASC, spg.game_name ASC LIMIT {$limit} OFFSET {$offset}";
    ...
    error_log("AJAX featured_sports_gamelist error: " . $e->getMessage());
}
