...
$combined_fishing_providers = $exa_providers;
...
if (!empty($current_provider_slug_initial) && !empty($current_provider_server_initial)) {
    foreach ($combined_fishing_providers as $provider) {
...
<section class="relative bg-[#000134] min-h-screen text-white">
	<div class="container mx-auto p-3 lg:pb-8 relative z-10">
			...
			<ol class="flex items-center pb-1 overflow-x-scroll whitespace-nowrap opacity-scroll">
				...
				<li class="inline-flex items-end pr-1 group">
					<div class="flex items-center">
                        ...
                        <a class="text-xs pl-1 border-b border-transparent hover:lg:border-primary transition-all duration-200 ease-in-out group-last:text-primary text-primary" href="fishing">fishing</a>
					</div>
				</li>
        ...
        <h2 class="text-xl font-bold mb-4" id="main-provider-heading">Pilih Provider Fishing:</h2>
        ...
        <div class="provider-scroll-container">
            ...
            <?php
            if (empty($combined_fishing_providers)) {
                echo '<div class="w-full text-center py-10"><p class="text-gray-400">Tidak ada provider Fishing yang tersedia saat ini di database. Pastikan Anda sudah menjalankan update provider di halaman admin.</p></div>';
            } else {
                foreach ($combined_fishing_providers as $provider) {
        ...
        <div id="provider-header-section" class="flex justify-between items-center lg:pl-3 mb-4" style="display: none;">
            ...
            <a href="fishing" class="btn btn-sm bg-primary text-white py-1 px-3 rounded-md hover:brightness-90 transition-all duration-200 ease-in-out">
        ...
        <h2 class="text-xl font-bold mb-4 mt-2" id="games-section-title">Game fishing Pilihan:</h2>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadGames(providerCode, providerServer, gameType, searchTermQuery = '', providerName = '', append = false) {
        ...
        if (providerCode === 'featured' && providerServer === 'gamexa_featured') {
            url = `ajax/featured_fishing_gamelist.php?${searchParam}${paginationParams}&_=${new Date().getTime()}`;
            ...
            url = `ajax/fishing_gamelist.php?provider_code=${encodeURIComponent(providerCode)}&game_type=${encodeURIComponent(gameType)}${searchParam}${paginationParams}&_=${new Date().getTime()}`;
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (providerCode !== 'featured') {
                        ...
                        if (gamesSectionTitle) gamesSectionTitle.textContent = 'Game fishing Pilihan:';
    ...
    if (providerScrollContainer) {
        providerScrollContainer.addEventListener('click', function(event) {
            ...
            if (targetLink) {
                ...
                const newUrl = `fishing?provider=${encodeURIComponent(providerCode)}&server=${encodeURIComponent(providerServer)}`;
    ...
    const backToProviderListLink = document.querySelector('#provider-header-section a[href="fishing"]');
