...
$combined_lottery_providers = $exa_providers;
...
if (!empty($current_provider_slug_initial) && !empty($current_provider_server_initial)) {
    foreach ($combined_lottery_providers as $provider) {
    ...
    if ($current_provider_slug_initial === 'featured' && $current_provider_server_initial === 'gamexa_featured') {
        $current_provider_name_initial = 'Rekomendasi GameXa';
    }
...
<section class="relative bg-[#000134] min-h-screen text-white">
	<div class="container mx-auto p-3 lg:pb-8 relative z-10">
			...
			<ol class="flex items-center pb-1 overflow-x-scroll whitespace-nowrap opacity-scroll">
				...
				<li class="inline-flex items-end pr-1 group">
					<div class="flex items-center">
                        ...
                        <a class="text-xs pl-1 border-b border-transparent hover:lg:border-primary transition-all duration-200 ease-in-out group-last:text-primary text-primary" href="lottery">lottery</a>
					</div>
				</li>
        ...
        <h2 class="text-xl font-bold mb-4" id="main-provider-heading">Pilih Provider Lottery</h2>
        ...
        <div class="provider-scroll-container">
            <a href="#" class="provider-scroll-item play-provider-link"
               ...
               data-provider-name="Rekomendasi GameXa"
               data-server="gamexa_featured"> <figure class="provider-scroll-image">
                    <img alt="Rekomendasi GameXa" loading="lazy" src="assets/img/recommendation-icon.png">
                </figure>
                <p class="provider-scroll-name">Rekomendasi</p>
            </a>
            <?php
            if (empty($combined_lottery_providers)) {
                echo '<div class="w-full text-center py-10"><p class="text-gray-400">Tidak ada provider lottery yang tersedia saat ini di database. Pastikan Anda sudah menjalankan update provider di halaman admin.</p></div>';
            } else {
                foreach ($combined_lottery_providers as $provider) {
        ...
        <div id="provider-header-section" class="flex justify-between items-center lg:pl-3 mb-4" style="display: none;">
            ...
            <a href="lottery" class="btn btn-sm bg-primary text-white py-1 px-3 rounded-md hover:brightness-90 transition-all duration-200 ease-in-out">
        ...
        <h2 class="text-xl font-bold mb-4 mt-2" id="games-section-title">Game Lottery Pilihan:</h2>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadGames(providerCode, providerServer, gameType, searchTermQuery = '', providerName = '', append = false) {
        ...
        if (providerCode === 'featured' && providerServer === 'gamexa_featured') {
            url = `ajax/featured_lottery_gamelist.php?${searchParam}${paginationParams}&_=${new Date().getTime()}`;
            if (!append) providerName = 'Rekomendasi GameXa';
            ...
            url = `ajax/lottery_gamelist.php?provider_code=${encodeURIComponent(providerCode)}&game_type=${encodeURIComponent(gameType)}${searchParam}${paginationParams}&_=${new Date().getTime()}`;
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (providerCode !== 'featured') {
                        ...
                        if (gamesSectionTitle) gamesSectionTitle.textContent = 'Game Lottery Pilihan';
    ...
    if (providerScrollContainer) {
        providerScrollContainer.addEventListener('click', function(event) {
            ...
            if (targetLink) {
                ...
                const newUrl = `lottery?provider=${encodeURIComponent(providerCode)}&server=${encodeURIComponent(providerServer)}`;
    ...
    const backToProviderListLink = document.querySelector('#provider-header-section a[href="lottery"]');
    if (backToProviderListLink) {
        backToProviderListLink.addEventListener('click', function(event) {
            ...
            loadGames('featured', 'gamexa_featured', pageGameType, '', 'Rekomendasi GameXa', false);
        ...
        if (provider && server) {
            ...
            if (currentProviderNameDisplay === '') {
                 ...
                 if (provider === 'featured' && server === 'gamexa_featured') {
                    currentProviderNameDisplay = 'Rekomendasi GameXa';
                 }
            }
            ...
            if (mainProviderHeading) mainProviderHeading.style.display = 'block';
            if (providerSearchSection) providerSearchSection.style.display = 'flex';
            if (gamesSectionTitle) gamesSectionTitle.textContent = 'Game lottery Pilihan GameXa:';
            if (toggleProviderViewBtn) toggleProviderViewBtn.style.display = 'flex'; 
            setProviderView(isListView); // Reapply stored view
