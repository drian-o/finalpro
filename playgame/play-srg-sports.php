...
// --- FUNGSI LOGGING ---
/**
 * Menulis pesan log ke file kustom logs/get_urls_srg_sports_game.log
...
function write_game_log($level, $message) {
    ...
    $log_file = $log_dir . 'get_urls_srg_sports_game.log'; // Nama log file khusus untuk Sportsbook
    ...
    $log_entry = "[{$timestamp}] [SRG SPORTS LAUNCH] [" . strtoupper($level) . "] " . $message . "\n";
...
// Asumsi: 'provider_code' adalah provider_code, 'game_code' adalah game_code
...
$provider_code = (int)$_GET['provider_code']; // Pastikan ini integer sesuai API SRG
$player_username = $_SESSION['nama_pengguna_anggota'];
$game_type = $_GET['game_type'] ?? 'SPORT_BOOK'; // Gunakan tipe yang benar untuk Sportsbook

write_game_log('INFO', "Mencoba membuka game SRG Sportsbook untuk Player: '{$player_username}', Provider Code: '{$provider_code}', Game Code: '{$game_code}', Game Type: '{$game_type}'");
...
if (is_string($launch_url) && !empty($launch_url)) {
    write_game_log('SUCCESS', "Game SRG Sportsbook berhasil dibuka untuk player '{$player_username}'. Mengalihkan ke URL game: " . $launch_url);
    header("Location:" . $launch_url);
    exit;
} else {
    $error_reason = 'URL game tidak didapatkan dari API atau respons API gagal.';
    write_game_log('ERROR', "Gagal membuka game SRG Sportsbook untuk player '{$player_username}'. Penyebab: {$error_reason}");
