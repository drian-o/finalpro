...
function write_game_log($level, $message) {
    ...
    $log_file = $log_dir . 'get_urls_srg_egames_game.log';
    ...
    $log_entry = "[{$timestamp}] [SRG EGAMES LAUNCH] [" . strtoupper($level) . "] " . $message . "\n";
...
$game_type = $_GET['game_type'] ?? 'OTHER'; 

write_game_log('INFO', "Mencoba membuka game SRG E-Games untuk Player: '{$player_username}', Provider Code: '{$provider_code}', Game Code: '{$game_code}', Game Type: '{$game_type}'");
...
if (is_string($launch_url) && !empty($launch_url)) {
    write_game_log('SUCCESS', "Game SRG E-Games berhasil dibuka untuk player '{$player_username}'. Mengalihkan ke URL game: " . $launch_url);
    header("Location:" . $launch_url);
    exit;
} else {
    $error_reason = 'URL game tidak didapatkan dari API atau respons API gagal.';
    write_game_log('ERROR', "Gagal membuka game SRG E-Games untuk player '{$player_username}'. Penyebab: {$error_reason}");
