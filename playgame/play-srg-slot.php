...
// --- FUNGSI LOGGING ---
/**
 * Menulis pesan log ke file kustom logs/get_urls_game.log
...
function write_game_log($level, $message) {
    ...
    $log_file = $log_dir . 'get_urls_srg_game.log'; // Ubah nama log file untuk SRG
    ...
    $log_entry = "[{$timestamp}] [SRG GAME LAUNCH] [" . strtoupper($level) . "] " . $message . "\n";
...
$game_type = $_GET['game_type'] ?? 'UNKNOWN'; 

write_game_log('INFO', "Mencoba membuka game SRG untuk Player: '{$player_username}', Provider Code: '{$provider_code}', Game Code: '{$game_code}', Game Type: '{$game_type}'");
...
if (is_string($launch_url) && !empty($launch_url)) {
    write_game_log('SUCCESS', "Game SRG berhasil dibuka untuk player '{$player_username}'. Mengalihkan ke URL game: " . $launch_url);
    header("Location:" . $launch_url);
    exit;
} else {
    $error_reason = 'URL game tidak didapatkan dari API atau respons API gagal.';
    write_game_log('ERROR', "Gagal membuka game SRG untuk player '{$player_username}'. Penyebab: {$error_reason}");
