...
$page_game_type = 'crash'; 

// Tentukan data provider "virtual" untuk halaman ini, karena tidak ada pilihan provider
$current_provider_slug_initial = 'all_crash_games'; // Slug fiktif untuk semua game crash
$current_provider_server_initial = 'gamexa_crash'; // Server fiktif untuk game crash
$current_provider_name_initial = 'Semua Game Crash'; // Nama tampilan
...
<style>
/* CSS UNTUK KOTAK PENCARIAN GAME (Diperbarui agar mirip dengan arcade dan cockfight) */
.game-search-box { 
    position: relative;
    margin-bottom: 1rem;
    display: flex; /* Ditambahkan untuk flex layout */
    align-items: center;
    gap: 0.5rem; /* Spasi antara input dan button (meskipun di crash tidak ada button) */
}
...
.game-grid-figure img {
    ...
    height: 100%;
    object-fit: cover;
    border-radius: 0.375rem 0.375rem 0 0; /* Menambahkan border-radius agar konsisten */
}
.game-grid-name {
    font-size: 0.75rem;
    font-weight: 500;
...
<section class="relative bg-[#000134] min-h-screen text-white">
	<div class="container mx-auto p-3 lg:pb-8 relative z-10">
			...
			<ol class="flex items-center pb-1 overflow-x-scroll whitespace-nowrap opacity-scroll">
				...
				<li class="inline-flex items-end pr-1 group">
					<div class="flex items-center">
                        ...
                        <a class="text-xs pl-1 border-b border-transparent hover:lg:border-primary transition-all duration-200 ease-in-out group-last:text-primary text-primary" href="crash">Crash</a>
					</div>
				</li>
        ...
        <h2 class="text-xl font-bold mb-4" id="main-provider-heading">Game Crash GameXa:</h2>
        
        <div id="game-search-section-main" class="game-search-box w-full mt-1 mb-3"> <i class="fas fa-search search-icon"></i> <input type="text" id="game-search-input-main" placeholder="Cari Game Crash..." value="<?php echo htmlspecialchars($search_term_initial); ?>">
        </div>

        <h2 class="text-xl font-bold mb-4 mt-2" id="games-section-title">Daftar Game Crash:</h2>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentProviderSlug = 'all_crash_games'; // Fiktif, mewakili semua game crash
    let currentProviderServer = 'gamexa_crash'; // Fiktif, mewakili server GameXa untuk crash
    let currentProviderNameDisplay = 'Semua Game Crash'; 
    ...
    function loadGames(providerCode, providerServer, gameType, searchTermQuery = '', providerName = '', append = false) {
        ...
        // Untuk halaman crash, kita selalu memanggil crash_gamelist.php
        url = `ajax/crash_gamelist.php?game_type=${encodeURIComponent(gameType)}&provider_code=${encodeURIComponent(providerCode)}${searchParam}${paginationParams}&_=${new Date().getTime()}`;
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (gamesSectionTitle) gamesSectionTitle.textContent = 'Daftar Game Crash:'; // Judul akan selalu ini
                } else {
                    if (!append) {
    ...
    function handleGameClick(event) {
        ...
        if (targetLink) {
            ...
            let basePlayUrl;
            if (server === 'gamexa_crash' || server === 'gamexa') { 
    ...
    const providerSearchSection = document.getElementById('provider-search-section'); // Ini tidak ada di crash.php, tapi untuk jaga-jaga
    const providerScrollContainer = document.querySelector('.provider-scroll-container'); // Ini tidak ada di crash.php, tapi untuk jaga-jaga
