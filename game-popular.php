<div class="w-full px-3 mt-3 lg:mt-5 order-6">
	<div class="flex justify-between items-center mb-4 lg:mb-3">
        <div class="flex-grow flex justify-center items-center">
            <p class="md:text-lg font-medium text-white text-center game-popular-title-container">
                <i class="mdi mdi-fire text-primary text-xl mr-2 animate-pulse-slow-icon"></i>
                Game Populer
            </p>
        </div>
	</div>
	<div class="mt-3 -mx-[5px] lg:-mx-2 pb-2 whitespace-nowrap overflow-x-scroll overflow-y-hidden lg:overflow-x-hidden opacity-scroll">
		<?php
		$query_game_populer = "SELECT * FROM gamepopuler ORDER BY id ASC LIMIT 6"; // MODIFIED QUERY
		$result_game_populer = mysqli_query($koneksi, $query_game_populer);

		if ($result_game_populer && mysqli_num_rows($result_game_populer) > 0) { // Check if query was successful and returned rows
			while ($row_game = mysqli_fetch_assoc($result_game_populer)) {
				$nama_game = $row_game['game_name'];
				$gambar_game = $row_game['banner'];
				$provider_game = $row_game['provideragent']; // Make sure this column exists in 'gamepopuler'
				$kode_game = $row_game['game_code'];       // Make sure this column exists in 'gamepopuler'
				$is_hot = isset($row_game['is_hot']) ? $row_game['is_hot'] : true; // Check for is_hot column, default to true
				$game_type = 'slot'; // Mengasumsikan game populer adalah tipe 'slot'. Sesuaikan jika diperlukan.
		?>
		...
		<?php
			...
			// echo "<p>No popular games available at the moment.</p>";
			if (!$result_game_populer) {
				// For debugging: echo "Error: " . mysqli_error($koneksi);
			}
		}
		?>
	</div>
...
<style>
    ...
    .game-popular-title-container .mdi {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadingIndicator = document.getElementById('popularGameLoadingIndicator');

    if (popularGameLinks.length > 0) {
        popularGameLinks.forEach(link => {
